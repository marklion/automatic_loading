...
static void set_log_level(std::ostream &out, std::vector<std::string> _params)
{
    ...
    check_resp += common_cli::check_params(_params, 1, "请输入日志模块(0-CONFIG,1-MODBUS_IO,2-STATE_MACHINE,3-LIDAR,4-XLRD,5-LIVE_CAREMA,6-HHT,7-PLATE_GATE,8-SCALE,9-DROP_SYSTEM,10-TEST):");
    if (!check_resp.empty())
    {
        out << check_resp << std::endl;
    }
