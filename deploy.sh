...
start_docker_con() {
    local CON_ID=`docker create --privileged --restart=always -e BASE_URL=${BASE_URL_INPUT} -v ${DATA_DIR_INPUT}:/database -p ${PORT_INPUT}:35511 ${DOCKER_IMG_NAME} /root/install.sh`
